...
To update the database use this command from the Web project folder (replace `Uber.Backend` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Uber.Backend.Infrastructure/Uber.Backend.Infrastructure.csproj -s Uber.Backend.Web.csproj
```
